...
internal sealed record ModelReceipt(string Source, string License, string Version, string[] Files,
    string? Name = null, string? Language = null);
...
public static class ModelPackage
{
    ...
    {
        if (string.IsNullOrWhiteSpace(relative)) throw new InvalidDataException("Package contains an empty file name.");
    ...
    public static ModelDetails Inspect(ModelPurpose purpose, string location)
    {
        ...
        {
            NoLinks(location);
            // Ownership survives missing/corrupt engine data. Collect every safely identified
            // existing file before validation; absence must not truncate the rest of the receipt.
            if (purpose == ModelPurpose.Ocr)
            {
                if (File.Exists(location)) files.Add(location);
            }
            else
            ...
            ModelReceipt? receipt = null;
            Exception? inventoryError = null;
            ...
            if (File.Exists(receiptPath))
            {
                ...
                try
                {
                    receipt = JsonSerializer.Deserialize<ModelReceipt>(ReadLimitedText(receiptPath, 16 * 1024 * 1024))
                    ...
                    {
                        // Validate the entire list before trusting any receipt-provided paths.
                        string[] ownedPaths = receipt.Files.Select(relative => SafePath(location, relative)).ToArray();
                        foreach (string owned in ownedPaths)
                        {
                            ...
                            else inventoryError ??= new InvalidDataException("An identified model file is missing: " + Path.GetFileName(owned));
                        }
                    }
                    if (!string.IsNullOrWhiteSpace(receipt.Name)) name = receipt.Name;
                    if (!string.IsNullOrWhiteSpace(receipt.Language)) language = receipt.Language;
                    ...
                    version = string.IsNullOrWhiteSpace(receipt.Version) ? "Unknown" : receipt.Version;
                }
            ...
            if (purpose == ModelPurpose.Ocr)
            {
                if (!File.Exists(location)) return new(name, purpose, language, version, location, ModelInstallState.Missing,
                    files.Sum(p => new FileInfo(p).Length), source, license, files, "OCR data is missing; any remaining receipt can be removed.");
                string code = Path.GetFileNameWithoutExtension(location);
                if (!IsLanguage(code) || !Ocr.OcrLanguageCatalog.IsSourceCode(code))
            ...
            else
            {
                ...
                if (name == "Unknown") name = receipt?.Name ?? Path.GetFileName(location);
                if (version == "Unknown") version = Text(metadata.RootElement, "package_version");
                if (source == "Unknown") source = Text(metadata.RootElement, "source");
                if (license == "Unknown") license = Text(metadata.RootElement, "license");
            ...
            if (inventoryError is not null) throw inventoryError;
            foreach (string owned in files)
            {
                // Optional files may be empty, but must still be readable.
