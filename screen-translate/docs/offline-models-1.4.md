...
States distinguish missing, checking, discovered, validated, invalid and read errors. OCR validation failures are shared between the main window and its model managers for the session. Successful native validation or a successfully installed, native-validated replacement clears the previous failure; failed replacements retain it. A temporary model-use block is reported separately and never cached as corrupt data. **Validated** is shown only following successful actual OCR loading; refresh conservatively returns to discovery. Translation packages are always unvalidated in this build. Main-window discovery badges now say **Discovered**.
...
`<language>.traineddata.screen-translate.json` and package-local `.screen-translate.json` receipts retain reviewed provenance and installed-file inventories. New receipts also retain the model name and language/direction so removal can identify damaged packages. Missing or corrupt engine data does not discard a safe receipt inventory. OCR receipts whose data files are missing remain visible for removal. Invalid receipt paths never authorize additional files. These files are management metadata, not language settings. They never make a package engine-validated.
...
Read-only model files block removal before anything moves. If deletion fails after files have moved, removal reports the failure and exact transaction location containing any remaining data, instead of claiming that disk space was reclaimed. Resolve the reported access problem before cleaning up that private transaction directory.
...
Recovery regressions cover missing metadata/weights/auxiliary files, malformed metadata and receipts, removal of orphan OCR receipts, read-only files and failed transaction cleanup, replacement after a remembered native failure, validation history shared through the actual modal manager, and switching languages while native validation is still active.
...
- Automated: `dotnet build` passed with zero warnings/errors. The required Debug acceptance command and Release CI script each passed 488 assertions, with genuine OCR fixtures and no skips.
- Rendered UI: inspected the manager in Light/Dark, at minimum size and synthetic 150% DPI, plus the download disclosure dialog and repaired/invalid OCR states. Metadata remains scrollable and actions remain accessible.
